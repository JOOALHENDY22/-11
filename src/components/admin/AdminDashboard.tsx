  ...
  return (
    <div className={`min-h-screen font-sans transition-colors duration-200 ${isDarkMode ? 'dark bg-[#090d16] text-slate-100' : 'bg-[#f8fafc] text-slate-800'}`}>
      ...
      <header className={`sticky top-0 z-40 backdrop-blur-xl border-b ${isDarkMode ? 'bg-[#090d16]/90 border-slate-800/80 shadow-sm' : 'bg-white/90 border-slate-200/80 shadow-xs'}`}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 sm:h-20 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 sm:w-11 sm:h-11 rounded-2xl bg-rose-600 flex items-center justify-center text-white shadow-sm">
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                <span className="text-[11px] font-semibold px-2.5 py-0.5 rounded-full bg-rose-500/10 text-rose-700 dark:text-rose-300 border border-rose-500/20">
                  {language === 'ar' ? 'لوحة الإدارة والاعتماد المركزية' : 'Admin Central Console'}
                </span>
              </div>
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            <div className="flex items-center gap-2.5 bg-slate-100/90 dark:bg-slate-900 border border-slate-200/90 dark:border-slate-800 px-3 py-1.5 rounded-2xl shadow-xs">
              <div className="w-8 h-8 rounded-xl bg-rose-600 text-white flex items-center justify-center text-xs font-bold">
                A
              </div>
              ...
              <span className="text-[10px] font-semibold px-2 py-0.5 rounded-lg bg-rose-500/10 text-rose-700 dark:text-rose-300 border border-rose-500/20">
                {language === 'ar' ? 'مدير المنظومة ✓' : 'SuperAdmin ✓'}
              </span>
            </div>

            <button
              onClick={onExitAdmin}
              className="px-3 py-2 rounded-xl text-xs font-semibold text-slate-700 dark:text-slate-200 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors flex items-center gap-1.5"
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8 animate-fade-in">
        ...
        {actionSuccessMsg && (
          <div className="p-4 rounded-2xl bg-emerald-500/10 border border-emerald-500/20 text-emerald-800 dark:text-emerald-300 text-xs flex items-center justify-between animate-slide-up shadow-sm">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4">
          ...
          <div className={`p-5 rounded-3xl border transition-all ${pendingUsers.length > 0 ? 'bg-amber-500/10 border-amber-500/30 text-amber-900 dark:text-amber-200' : 'bg-white dark:bg-slate-900 border-slate-200/90 dark:border-slate-800'}`}>
            <div className="flex items-center justify-between mb-2">
              <Clock className={`w-5 h-5 ${pendingUsers.length > 0 ? 'text-amber-600 animate-pulse' : 'text-slate-400'}`} />
              {pendingUsers.length > 0 && (
                <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-amber-500 text-white">
                  {language === 'ar' ? 'يتطلب إجراء' : 'Action Req.'}
                </span>
              )}
            </div>
            <span className="text-3xl font-extrabold block text-slate-900 dark:text-white">
              {pendingUsers.length}
            </span>
          ...
          {/* Active Doctors */}
          <div className="p-5 rounded-3xl bg-white dark:bg-slate-900 border border-slate-200/90 dark:border-slate-800">
        ...
        <div className="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200/90 dark:border-slate-800 shadow-card p-6 sm:p-8 space-y-6">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-slate-100 dark:border-slate-800">
            <div>
              <h3 className="text-lg font-bold text-slate-900 dark:text-white flex items-center gap-2">
                <Users className="w-5 h-5 text-rose-600" />
                <span>{language === 'ar' ? 'إدارة المستخدمين وطلبات الاعتماد' : 'User Accounts & Approvals'}</span>
              </h3>
              <p className="text-xs text-slate-400 mt-0.5">
                {language === 'ar' ? 'مراجعة الأطباء والصيادلة، وتفعيل أو إيقاف أي حساب فورياً' : 'Review doctors, pharmacists, approve or freeze accounts'}
              </p>
            </div>
            ...
            <div className="flex items-center gap-2.5">
              <button
                ...
                className="p-2.5 rounded-xl border border-slate-200 dark:border-slate-700 hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-600 dark:text-slate-300 transition-colors shadow-xs"
          ...
          <div className="flex flex-col md:flex-row items-center justify-between gap-4">
            {/* Filter Tabs */}
            <div className="flex flex-wrap items-center gap-1.5 p-1 rounded-2xl bg-slate-100 dark:bg-slate-800 border border-slate-200/80 dark:border-slate-700 w-full md:w-auto">
              <button
              ...
              <button
                onClick={() => setStatusFilter('patient')}
                className={`px-3 py-1.5 rounded-xl text-xs font-semibold transition-all ${
                  statusFilter === 'patient'
                    ? 'bg-teal-600 text-white shadow-xs'
                    : 'text-slate-500 hover:text-slate-900 dark:hover:text-white'
                }`}
              >
